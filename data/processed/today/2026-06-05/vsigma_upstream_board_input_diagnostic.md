...
- objective_availability_gate | status=EMPTY_REQUIRED | rows=0 | date=NO_ROWS | path=data/processed/today/2026-06-05/vsigma_objective_availability_gate.csv | detail=required upstream file exists but has zero rows
- context_adjusted_final_picks | status=EMPTY_REQUIRED | rows=0 | date=NO_ROWS | path=data/processed/today/2026-06-05/vsigma_context_adjusted_final_picks.csv | detail=required upstream file exists but has zero rows
...
- match_stat_forecasts | status=EMPTY_REQUIRED | rows=0 | date=NO_ROWS | path=data/processed/today/2026-06-05/vsigma_match_stat_forecasts.csv | detail=required upstream file exists but has zero rows
- forecast_market_translator | status=EMPTY_REQUIRED | rows=0 | date=NO_ROWS | path=data/processed/today/2026-06-05/vsigma_forecast_market_translator.csv | detail=required upstream file exists but has zero rows
- daily_execution_board | status=OK | rows=1 | date=OK | path=data/processed/today/2026-06-05/vsigma_daily_execution_board.csv | detail=file has rows and passed basic diagnostic checks
...
- probable_lineup_consensus | status=OK | rows=1 | date=OK | path=data/processed/today/2026-06-05/vsigma_probable_lineup_consensus.csv | detail=file has rows and passed basic diagnostic checks
