...
- P1 | SHADOW_CANDIDATE_REQUIRED | MODEL_SHADOW_PROPOSAL | MARKET_RISK_CLUSTER | n=4 | auto_apply=NO | action=Create a shadow-only candidate for pattern OVER_2_5::FAILURE_MODE_LOW_CONVERSION; do not change production until backtest/forward-test promotion gates pass.
...
- P1 | PROPOSAL_ONLY | OPERATIONAL_PROPOSAL | EXPIRED_PRELOCK_CLUSTER | n=4 | auto_apply=NO | action=Review execution timing and exclude expired rows from predictive hit-rate metrics.
...
- P2 | PROPOSAL_ONLY | DATA_QUALITY_PROPOSAL | UNRESOLVED_DOMINANCE | n=18 | auto_apply=NO | action=Resolve UNKNOWN/UNRESOLVED/NO_SIGNAL evidence quality before creating any model shadow candidate.
