...
## Cards
- MOBILE_STATUS | status=WARN | betting_permission=NO; final_decision=WATCH_ONLY_NO_STAKE; action_level=WATCH; alert_route=NO_ALERT | next=Review sanity/guard warnings before using learning outputs.
- OPERATOR | status=WATCH | final_decision=WATCH_ONLY_NO_STAKE; alert_route=NO_ALERT; sanity=PASS | next=Read operator brief only if status is REVIEW_NOW/LIVE/STOP.
