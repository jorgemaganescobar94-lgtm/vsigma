...
- Decision outcome ledger rows total: 35
- Decision outcome ledger actionable rows: 6
- Decision outcome ledger non-actionable rows: 29
- Decision outcome ledger no bet rows: 16
- Decision outcome ledger expired rows: 14
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1545418 | Bundesliga | SC Paderborn 07 | VfL Wolfsburg | OVER_1_5 | NO_BET | NO | KICKOFF_ALREADY_PASSED | NO |  |  | EXPIRED |  | KICKOFF_ALREADY_PASSED |  |
| 1494178 | Allsvenskan | IF Elfsborg | BK Hacken | OVER_2_5 | NO_BET | NO | KICKOFF_ALREADY_PASSED | NO |  |  | EXPIRED |  | PRELOCK_CONFIRMED |  |
...
- odds structure depth: target_fixtures: 5; OK: 5; RICH_MIXED: 3; RICH_COHERENT: 2; BROAD_GOALS: 2
...
| P2 | api_data | Target API enrichment to candidate fixtures only | Coverage gaps are present, but broad calendar enrichment would add cost and repo churn. | Improves prelock evidence where it matters without expanding data volume unnecessarily. | Medium; API quotas and cache growth must be controlled. | Fetch lineups only for candidate picks in-window, injuries only for reliable leagues, and fixture statistics only for TOP candidates; keep cache bounds. | YES | prelock_odds_state=ODDS_NOT_AVAILABLE: 1 |
...
## Input Inventory
- generated_at: 2026-05-25T23:38:54+01:00
