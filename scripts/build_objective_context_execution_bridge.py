...
def build(target_date: str, timezone: str, processed: Path) -> list[dict[str, str]]:
    ...
    return [row for row in out if row.get("fixture_id") and row.get("home_team") and row.get("away_team")]


def md(target_date: str, rows: list[dict[str, str]]) -> str:
    lines = [
    ...
    if not rows:
        lines.append("- none. Real objective context gate has no bridgeable rows.")
    for row in rows:
        lines.append(
...
def run(target_date: str, timezone: str, processed: Path) -> None:
    ...
    for base in [processed / "today" / target_date, processed / "governance"]:
        ...
        (base / "vsigma_objective_context_execution_bridge.md").write_text(md(target_date, rows), encoding="utf-8")
