...
FIELDS = [
    ...
    "impact_weighting_status","unit_weighting_status","home_xi_strength","away_xi_strength","xi_edge","home_unit_team_index","away_unit_team_index","unit_edge",
    "home_raw_attack_index","away_raw_attack_index","home_style_attack_index","away_style_attack_index","home_attack_index","away_attack_index",
    "home_defense_index","away_defense_index","home_control_index","away_control_index","home_set_piece_index","away_set_piece_index","draw_risk_index","goal_suppression_index",
...
SUMMARY_FIELDS = ["target_date","generated_at","query_home","query_away","rows","stat_confidence","impact_weighting_status","unit_weighting_status","next_action","auto_apply","production_change"]
OFFENSIVE_ROLES={"STRIKER","STRIKER_PAIR","WINGER","WIDE_MIDFIELD","ATTACKING_MIDFIELD","INTERIOR"}
FINISHER_ROLES={"STRIKER","STRIKER_PAIR"}
CREATOR_ROLES={"INTERIOR","ATTACKING_MIDFIELD","CENTRAL_MIDFIELD"}
WIDE_ROLES={"WINGER","WIDE_MIDFIELD","WING_BACK","FULL_BACK"}
DEFENSIVE_ROLES={"GOALKEEPER","CENTER_BACK","FULL_BACK","WING_BACK","DEFENSIVE_MIDFIELD"}
CENTER_BACK_ROLES={"CENTER_BACK"}
FULLBACK_ROLES={"FULL_BACK","WING_BACK"}
...
def style_lookup(style_rows): return {clean(r.get("team_side")):r for r in style_rows}
def role_values(players, side, field, roles=None):
    vals=[]
    for p in players.get(side,[]):
        if roles and s(p.get("tactical_role")) not in roles: continue
        vals.append(fnum(p.get(field)))
    if vals: return vals
    return [fnum(p.get(field)) for p in players.get(side,[])]
def role_avg(players, side, field, roles=None): return avg(role_values(players,side,field,roles))
def style_attack_index(style):
    shots=clamp(fnum(style.get("avg_shots"))*5.0,20,85)
    sot=clamp(fnum(style.get("avg_sot"))*14.0,15,85)
    goals=clamp(fnum(style.get("avg_goals_for"))*37.5,10,88)
    corners=clamp(fnum(style.get("avg_corners"))*10.0,15,80)
    poss=clamp(fnum(style.get("avg_possession")),30,70)
    return round(shots*0.25+sot*0.25+goals*0.25+corners*0.15+poss*0.10,1)
def style_defense_index(style):
    ga_score=clamp(88 - fnum(style.get("avg_goals_against"))*35.0,25,90)
    yc_discipline=clamp(76 - max(0,fnum(style.get("avg_yellows"))-1.5)*7.0,50,80)
    compact_bonus=7 if "COMPACT" in s(style.get("defense_style")) or s(style.get("usual_shape")).startswith("5-") else 0
    fouls_control=clamp(72 - max(0,fnum(style.get("avg_fouls"))-12)*2.0,50,78)
    return round(clamp(ga_score*0.55+yc_discipline*0.20+fouls_control*0.15+compact_bonus,20,92),1)
def unit_attack_index(players, side, style):
    fin=role_avg(players,side,"attack_impact",FINISHER_ROLES)
    cre=role_avg(players,side,"attack_impact",CREATOR_ROLES)
    wide=role_avg(players,side,"attack_impact",WIDE_ROLES)
    ctl=role_avg(players,side,"control_impact",CONTROL_ROLES)
    sp=role_avg(players,side,"set_piece_impact")
    player_unit=round(fin*0.34+cre*0.26+wide*0.18+ctl*0.12+sp*0.10,1)
    st=style_attack_index(style)
    return round(player_unit*0.58+st*0.42,1), player_unit, st
def unit_defense_index(players, side, style):
    gk=role_avg(players,side,"defense_impact",{"GOALKEEPER"})
    cb=role_avg(players,side,"defense_impact",CENTER_BACK_ROLES)
    fb=role_avg(players,side,"defense_impact",FULLBACK_ROLES)
    dm=role_avg(players,side,"defense_impact",{"DEFENSIVE_MIDFIELD"})
    st=style_defense_index(style)
    player_unit=round(gk*0.22+cb*0.32+fb*0.18+dm*0.14+st*0.14,1)
    return round(player_unit,1)
def unit_control_index(players, side, style):
    mid=role_avg(players,side,"control_impact",CONTROL_ROLES)
    wide=role_avg(players,side,"control_impact",WIDE_ROLES)
    poss=clamp(fnum(style.get("avg_possession")),30,70)
    return round(mid*0.62+wide*0.13+poss*0.25,1)
def impact_context(day, base, file_slug):
    ...
    if not player_rows:
        return {"impact_weighting_status":"NO_PLAYER_IMPACT_DATA","unit_weighting_status":"NO_UNIT_INDEX","home_xi_strength":0,"away_xi_strength":0,"xi_edge":0,"home_unit_team_index":0,"away_unit_team_index":0,"unit_edge":0,"home_raw_attack_index":0,"away_raw_attack_index":0,"home_style_attack_index":0,"away_style_attack_index":0,"home_attack_index":0,"away_attack_index":0,"home_defense_index":0,"away_defense_index":0,"home_control_index":0,"away_control_index":0,"home_set_piece_index":0,"away_set_piece_index":0,"draw_risk_index":50,"goal_suppression_index":50,"home_compact_block":False,"away_compact_block":False,"note":"No player-impact file available; forecast uses base model only."}
    players={"home":[],"away":[]}
    for r in player_rows:
        side=clean(r.get("team_side"))
        if side in players: players[side].append(r)
    styles=style_lookup(style_rows)
    home_style=styles.get("home",{}); away_style=styles.get("away",{})
    def strength(side): return avg([fnum(p.get("player_impact_score")) for p in players[side]])
    h_str=strength("home"); a_str=strength("away"); xi_edge=round(h_str-a_str,1)
    h_raw_atk=role_avg(players,"home","attack_impact",OFFENSIVE_ROLES); a_raw_atk=role_avg(players,"away","attack_impact",OFFENSIVE_ROLES)
    h_atk,_,h_style_atk=unit_attack_index(players,"home",home_style); a_atk,_,a_style_atk=unit_attack_index(players,"away",away_style)
    h_def=unit_defense_index(players,"home",home_style); a_def=unit_defense_index(players,"away",away_style)
    h_ctl=unit_control_index(players,"home",home_style); a_ctl=unit_control_index(players,"away",away_style)
    h_sp=role_avg(players,"home","set_piece_impact"); a_sp=role_avg(players,"away","set_piece_impact")
    h_unit=round(h_atk*0.32+h_def*0.30+h_ctl*0.25+h_sp*0.13,1)
    a_unit=round(a_atk*0.32+a_def*0.30+a_ctl*0.25+a_sp*0.13,1)
    unit_edge=round(h_unit-a_unit,1)
    home_compact="COMPACT" in s(home_style.get("defense_style")) or s(home_style.get("usual_shape")).startswith("5-")
    away_compact="COMPACT" in s(away_style.get("defense_style")) or s(away_style.get("usual_shape")).startswith("5-")
    draw_risk=50 + (a_def-h_atk)*0.42 + max(0,-unit_edge)*0.85 + (8 if away_compact else 0) + (3 if a_sp>h_sp+3 else 0)
    suppression=50 + (a_def-h_atk)*0.48 + (8 if away_compact else 0) - max(0,h_atk-70)*0.25
    return {"impact_weighting_status":"PLAYER_IMPACT_WEIGHTED","unit_weighting_status":"UNIT_WEIGHTED_TEAM_INDEX","home_xi_strength":h_str,"away_xi_strength":a_str,"xi_edge":xi_edge,"home_unit_team_index":h_unit,"away_unit_team_index":a_unit,"unit_edge":unit_edge,"home_raw_attack_index":h_raw_atk,"away_raw_attack_index":a_raw_atk,"home_style_attack_index":h_style_atk,"away_style_attack_index":a_style_atk,"home_attack_index":h_atk,"away_attack_index":a_atk,"home_defense_index":h_def,"away_defense_index":a_def,"home_control_index":h_ctl,"away_control_index":a_ctl,"home_set_piece_index":h_sp,"away_set_piece_index":a_sp,"draw_risk_index":round(clamp(draw_risk,20,90),1),"goal_suppression_index":round(clamp(suppression,20,90),1),"home_compact_block":home_compact,"away_compact_block":away_compact,"note":"Forecast adjusted by unit-weighted team index: player roles + team production + style context."}
...
def stat_model(r, impact):
    raw_hp=fnum(r.get("home_prob")); raw_dp=fnum(r.get("draw_prob")); raw_ap=fnum(r.get("away_prob")); hp,dp,ap=raw_hp,raw_dp,raw_ap
    fav_gap=hp-ap; goal=s(r.get("goal_profile")); scen=s(r.get("scenario")); xi=s(r.get("xi_status")); hs=s(r.get("home_shape")); aw=s(r.get("away_shape")); result=s(r.get("result_forecast")); scorelines=s(r.get("scorelines"))
    total_xg=2.35
    ...
    if impact.get("impact_weighting_status") == "PLAYER_IMPACT_WEIGHTED":
        edge=fnum(impact.get("unit_edge")); h_atk=fnum(impact.get("home_attack_index")); a_atk=fnum(impact.get("away_attack_index")); h_def=fnum(impact.get("home_defense_index")); a_def=fnum(impact.get("away_defense_index")); draw_risk=fnum(impact.get("draw_risk_index")); suppression=fnum(impact.get("goal_suppression_index"))
        home_prob_delta=clamp(edge*0.0025 + (h_atk-a_def)*0.0010, -0.040, 0.035)
        away_prob_delta=clamp(-edge*0.0016 + (a_atk-h_def)*0.0008, -0.025, 0.025)
        ...
        hxg_delta=clamp((h_atk-a_def)*0.006 + edge*0.008, -0.22, 0.18)
        axg_delta=clamp((a_atk-h_def)*0.004 - edge*0.003, -0.14, 0.14)
        total_xg_delta=clamp((50-suppression)*0.004, -0.20, 0.10)
    ...
    btts=pct(100*(1-exp_neg(hxg))*(1-exp_neg(axg))); over15=pct(100*(1-exp_neg(total_xg)*(1+total_xg))); over25=pct(100*(1-exp_neg(total_xg)*(1+total_xg+(total_xg**2)/2))); under35=pct(100*(exp_neg(total_xg)*(1+total_xg+(total_xg**2)/2+(total_xg**3)/6)))
    home_cs=pct(100*exp_neg(axg)); away_goal=pct(100*(1-exp_neg(axg)))
    ft_primary=choose_score(hxg,axg,hp,dp,ap,under35,scorelines)
    goal_profile="LOW_TO_MODERATE_GOALS" if total_xg < 2.15 else ("MODERATE_GOALS" if total_xg < 2.55 else "OPEN_GOALS")
    ...
    home_sot=clamp(round(hxg*2.7),1,8); away_sot=clamp(round(axg*2.6),1,6); home_bc=clamp(round(hxg*1.35),0,4); away_bc=clamp(round(axg*1.20),0,3)
    ...
    ht="0-0 / 1-0" if home_first + away_first < 1.0 else ("1-0" if home_first > away_first + 0.30 else "0-0 / 1-1")
    first_team = r.get("home_team") if hp >= ap + 0.15 else (r.get("away_team") if ap >= hp + 0.15 else "NO_CLEAR_EDGE"); first_window = "25-55" if first_team != "NO_CLEAR_EDGE" else "NO_CLEAR_WINDOW"
    ...
    if "FAVORITE_CONTROL" in scen:
        fh="Home control, away compact; first goal value high; HT likely 0-0 or 1-0."; sh="If home scores first, game management; if 0-0 after 60', draw branch rises sharply."; base_branch="Home territorial control against compact away block."; danger_branch="0-0 after 60' or away transition/set-piece keeps draw alive."
    else:
...
def run(day,home,away,tz,base):
    ...
    print(f"Ad hoc unit-weighted full match projection built rows={len(out)}")
